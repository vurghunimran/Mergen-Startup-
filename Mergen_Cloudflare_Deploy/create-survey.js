...
async function handleLaunch() {
    ...
    try {
        ...
        // Note: We need to serialize questions if we are storing them in 'description' or a separate JSON column.
        // For simplicity in this demo schema, we might not have a 'questions' column in the `surveys` table definition I gave earlier (it had title, description, user_id).
        // I should have added a jsonb column `questions`. 
        // For now, let's assume `description` might store it or we just store metadata.
        // **Checking Schema**: Schema had `title, description, target_audience, question_count, respondents_limit`. No `questions` column on `surveys`. 
        // Real app would put questions in a separate table or JSON column.
        // Let's store questions in LocalStorage for "View Details" to still work in Prototype mode, 
        // OR we can't fully support detailed report generation from DB without storing questions.
        // I will stick to LocalStorage for the *questions content* if using the simple schema, 
        // OR update `MergenDB` to just log it.
        ...
        // 3. Fallback / Parallel: Save to LocalStorage (So the prototype always works flawlessly)
        const newSurvey = {
            id: 'srv_' + Date.now(),
            ...surveyPayload,
            status: 'Active',
